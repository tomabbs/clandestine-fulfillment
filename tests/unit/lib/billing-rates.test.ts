...
function createMockSupabase(defaultRule: Record<string, unknown> | null, override: Record<string, unknown> | null) {
...
describe("getEffectiveRate", () => {
  it("returns null when no default rule exists", async () => {
    ...
    const result = await getEffectiveRate(supabase as any, "ws-1", "org-1", "storage", "2026-03-01");
  ...
  it("returns override rate when override exists", async () => {
    const supabase = createMockSupabase(
      { id: "rule-1", amount: 0.25, rule_name: "Storage" },
      { override_amount: 0.10 },
    );
    const result = await getEffectiveRate(supabase as any, "ws-1", "org-1", "storage", "2026-03-01");
    expect(result).toEqual({ amount: 0.10, source: "override", ruleName: "Storage" });
  });
  ...
  it("handles string amounts by converting to number", async () => {
    ...
    const result = await getEffectiveRate(supabase as any, "ws-1", "org-1", "per_shipment", "2026-03-01");
