...
### Label Orders (inventory sync cutover — 2026-05-06)

- File: `src/actions/label-orders.ts`
- Key exports (all staff-only via `requireStaff()`):
  - `createLabelOrder({ orgId, items, notes? })` creates a real `warehouse_orders` row with `source='label_order'`, inserts `warehouse_order_items`, opens `inventory_commitments` through `commitOrderItems`, and decrements available inventory through `recordInventoryChange({ source:'label_order' })` using stable per-line correlation IDs.
  - `fulfillLabelOrder({ orderId })` marks the order fulfilled and releases commitments only; it does not decrement inventory a second time.
  - `voidLabelOrder({ orderId, reason? })` marks the order cancelled, releases commitments, and re-credits each item through `recordInventoryChange({ source:'label_order' })` with `label-order-void:*` correlation IDs.
- Companion test: `tests/unit/actions/label-orders.test.ts`.
...
- Key exports:
  ...
  - preorder tools: `getPreorderProducts`, `getBandcampProductDetectionDashboard` (read-only dashboard model for recent Bandcamp products from Bandcamp `release_date` / `new_date` windows plus preorder-signal buckets from `bandcamp_release_date` / `bandcamp_new_date` / `bandcamp_is_preorder`; mapping `created_at` is not treated as product age. UI date evidence is source-labeled: release dates are release evidence and `new_date` is only listing/detection evidence. `getPreorderProducts` feeds the single variant-authoritative Upcoming Releases card across a 90-day horizon and exposes SKU, display format, artist + album + Bandcamp package title without added display quotes, Bandcamp URL, available stock, Bandcamp sold count from Merch API raw `quantity_sold`, explicit pending order count/unit labels, and sample live Bandcamp `BC-{payment_id}` order numbers when `get_orders` resolves by SKU or package/option fallback. Product Detection only renders current-upcoming Bandcamp signals when they are missing from Upcoming Releases, plus recent/listed and stale historical audit rows. Display format prefers SKU prefix / Bandcamp type evidence over stale variant `format_name`), `manualRelease` (triggers `preorder-release-variant` for single-variant release — NOT the full fulfillment job), `getPreorderAllocationPreview`
