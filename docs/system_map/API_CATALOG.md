...
- Key exports:
  ...
  - preorder tools: `getPreorderProducts`, `getBandcampProductDetectionDashboard` (read-only dashboard model for recent Bandcamp products from Bandcamp `release_date` / `new_date` windows plus preorder-signal buckets from `bandcamp_release_date` / `bandcamp_new_date` / `bandcamp_is_preorder`; mapping `created_at` is not treated as product age. UI date evidence is source-labeled: release dates are release evidence, `new_date` is only listing/detection evidence, and future-release preorders are excluded from the recent/listed bucket so listing dates cannot be mistaken for release dates. Current-upcoming Bandcamp signals are displayed in their own dashboard section, while `getPreorderProducts` feeds the variant-authoritative Upcoming Releases card across a 90-day horizon), `manualRelease` (triggers `preorder-release-variant` for single-variant release — NOT the full fulfillment job), `getPreorderAllocationPreview`
