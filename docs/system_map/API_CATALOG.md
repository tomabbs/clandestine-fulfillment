...
- Key exports:
  ...
  - preorder tools: `getPreorderProducts`, `getBandcampProductDetectionDashboard` (read-only dashboard model for Bandcamp first-seen/new products from `bandcamp_product_mappings.created_at` plus preorder-signal buckets from `bandcamp_release_date` / `bandcamp_new_date` / `bandcamp_is_preorder`), `manualRelease` (triggers `preorder-release-variant` for single-variant release — NOT the full fulfillment job), `getPreorderAllocationPreview`
