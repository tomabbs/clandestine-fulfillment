...
- slug: sku-autonomous-phase1-apply-outcome-transition-rpc
  title: "Phase 1 residual — apply_sku_outcome_transition RPC wrapper + test (SKU-AUTO-14 full)"
  due_date: 2026-05-10
  severity: high
  context: "Phase 1 landed the PURE state-machine primitives 2026-04-26 (src/lib/server/sku-outcome-transitions.ts: STORED_IDENTITY_OUTCOME_STATES, FULL_OUTCOME_STATES, LEGAL_TRANSITIONS, isLegalTransition, validateOutcomeTransition; drift-guard test at tests/unit/lib/server/sku-outcome-transitions.test.ts — the SKU-AUTO-6 enforcer). DEFERRED in this pass: the DB-side RPC that actually mutates client_store_product_identity_matches.outcome_state + state_version under pg_advisory_xact_lock + writes the sku_outcome_transitions audit row in the same transaction. Action: (1) add `create or replace function apply_sku_outcome_transition(p_identity_match_id uuid, p_expected_state_version int, p_to_state text, p_trigger text, p_reason_code text, p_actor_kind text, p_actor_id uuid, p_run_id uuid, p_decision_id uuid, p_metadata jsonb) returns jsonb language plpgsql security definer` — MUST take pg_advisory_xact_lock(hashtext('sku_transition:'||p_identity_match_id)) BEFORE re-reading the row (SKU-AUTO-22), SELECT the current outcome_state + state_version, raise `stale_state_version` if `state_version != p_expected_state_version`, raise `illegal_transition` if (from_state, to_state, trigger) is not in the client-side legal table, UPDATE the identity row with `state_version = state_version + 1`, INSERT the sku_outcome_transitions row, RETURN the new state + new state_version; (2) thin TypeScript wrapper `applyOutcomeTransition()` in src/lib/server/sku-outcome-transitions.ts that calls the RPC and narrows the result + re-validates using `validateOutcomeTransition()` as a defense-in-depth; (3) concurrent-RPC integration test firing N simultaneous transitions on the same identity row — exactly ONE succeeds, N-1 return `stale_state_version` (SKU-AUTO-22 evidence). The lock key mirrors promote_identity_match_to_alias (already shipped in the Phase 0 migration) so both RPCs contend on the same per-identity advisory lock."
- slug: sku-autonomous-phase2-normalized-order-adapter
  title: "Phase 2 — loadNormalizedOrder adapter + evaluateOrderForHold + rankSkuCandidates CandidateEvidence"
  due_date: 2026-05-20
  severity: high
  context: "Phase 2 plumbing that the autonomous runner needs before the first dry-run can execute. Three helpers, all pure + fixture-tested, all new src/lib/server/* files: (1) `loadNormalizedOrder(connectionId, externalOrderId)` — single adapter that takes a webhook-path OR poll-path raw order object, hydrates from client_store_orders + client_store_order_lines, and returns NormalizedClientStoreOrder (typed union over Shopify/Woo/Squarespace line shapes). Both the webhook ingress (src/trigger/tasks/process-client-store-webhook.ts) and the poll ingress MUST use this adapter so order-hold evaluation is symmetric across paths (SKU-AUTO-3). (2) `evaluateOrderForHold(order: NormalizedClientStoreOrder, ctx: { warehouseInventory, activeAliases, identityMatches, fetchStatus })` — returns `{ holdDecision: 'hold' | 'partial_hold' | 'pass', heldLines: Array<{lineId, reason, missingSku?, placeholderSku?, fetchIncomplete?}>, commitableLines: Array<{lineId, variantId, qty}>, reasonCode }`. Reason-code enum: `unknown_remote_sku`, `placeholder_remote_sku` (uses isPlaceholderSku from src/lib/shared/utils.ts), `non_warehouse_match`, `fetch_incomplete_at_match`, `all_lines_warehouse_ready`. Every reason-code must round-trip through validateOutcomeTransition for the identity row it references. (3) `rankSkuCandidates(remote: NormalizedRemoteListing, warehouseCandidates: CandidateInput[], ctx: { stockSignals, stockHistory, clockSkew })` → `Array<{candidate, score, tier, evidence: CandidateEvidence, disqualifiers: DisqualifierCode[]}>`. CandidateEvidence is the audit structure that sku_autonomous_decisions.evidence_json persists — include: exact-SKU match, exact-barcode match, artist/vendor text similarity, canonical/Bandcamp title similarity, format match via parseMusicVariantDescriptors, variant/option text match, price delta, warehouse stock ATP, remote stock ATP (with tier from classifyStockTier), stock-stability check via isStockStableFor('tiebreak', ...). MUST refuse stock-based tiebreaks when warehouse tier ≠ 'authoritative' or remote tier ∈ {cached_only, unknown} (SKU-AUTO-7) and when fresh_remote_unbounded is present (SKU-AUTO-7). MUST refuse promotion-path consideration for isPlaceholderSku remote SKUs. Tests land in tests/unit/lib/server/{normalized-order,evaluate-order-for-hold,rank-sku-candidates}.test.ts — target >90% coverage on the ranking disqualifier matrix. Once landed, the grep guard for 'identity rows not read in fanout' graduates from standalone shell script to a positive integration fixture (SKU-AUTO-1 upgrade)."
