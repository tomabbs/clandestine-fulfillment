...
| SKU-2 | SKU matching schema/RPC substrate is present on the linked production database. | `supabase migration list --linked` shows `20260425000002_sku_matching_provenance` and `20260428000005_fix_alias_scope_trigger_org_guard`; `supabase db push --yes --include-all` completed successfully; `persist_sku_match`, `find_remote_to_canonical_dupes`, and `find_canonical_sku_duplicates` are callable. The shared `enforce_identity_match_scope()` trigger must not dereference `NEW.org_id` directly because `client_store_sku_mappings` has no `org_id` column. |
...
| SKU-MATCHING-MANUAL-1 | Review drawer supports manual remote candidate search and negative evidence suppression. | `supabase migration list --linked` shows `20260428000006_sku_match_candidate_rejections`; `tests/unit/app/admin/settings/sku-matching-client.test.tsx` covers searching the remote catalog, selecting a result, confirming it as a manual strong match, and marking the current candidate as not-a-match; `tests/unit/actions/sku-matching.test.ts` verifies suppression uses `sku_match_candidate_rejections` and does not write the live alias table directly. |
