...
| SKU-AUTO-3 | Webhook-ingest and poll-ingest both construct `NormalizedClientStoreOrder` via the shared adapter and call `evaluateOrderForHold()`, OR the poll path is explicitly disabled for any connection with `non_warehouse_order_hold_enabled=true`. | Pending-Phase-4 | Shared primitives `loadNormalizedOrder()` + `evaluateOrderForHold()` + `apply_order_fulfillment_hold` RPC are all live and pure-unit covered (`tests/unit/lib/server/normalized-order-loader.test.ts`, `order-hold-evaluator.test.ts`, `order-hold-policy.test.ts`, `order-hold-rpcs.test.ts`). The ingress wiring that invokes them from `src/trigger/tasks/process-client-store-webhook.ts` + `client-store-order-detect.ts` has not yet landed — that is the remaining Phase 4 piece. Flip to Active once both ingress paths import `loadNormalizedOrder` + call `evaluateOrderForHold` (and stamp `apply_order_fulfillment_hold` when hold decision fires) with a parity fixture test. |
...
| SKU-AUTO-18 | Dry-run mode (Phase 1) writes `sku_autonomous_runs` + `sku_autonomous_decisions` but writes nothing to identity tables, alias tables, `warehouse_orders.fulfillment_hold`, or the alert task queue. | Pending-Phase-1 | The `sku_autonomous_runs.dry_run` column exists (Phase 0 migration) and the telemetry summarizer in `src/lib/server/sku-autonomous-telemetry.ts` correctly excludes dry-run rows from production metrics (`r.dry_run === true` filter at line 592 of `src/trigger/tasks/sku-autonomous-telemetry.ts`). What is still missing is the dry-run RUNNER itself: no active task writes `dry_run: true` rows today (every `sku_autonomous_runs` insert across `sku-shadow-promotion`, `sku-holdout-stop-condition-sweep`, `sku-autonomous-telemetry`, and `webhook-rehydrate` hardcodes `dry_run: false`). A dedicated `sku-autonomous-dry-run` task that replays live evidence through the matcher + alias-promotion wrappers with every write short-circuited at the client-side would close this gate. Flip to Active once that task lands with a test fixture asserting zero identity-table / alias-table / hold-column writes. |
...
| SKU-AUTO-21 | When a held order contains valid warehouse-stocked lines, those lines are committed through `commitOrderItems()` in the same transaction as the hold write. | Pending-Phase-4 | Primitive wiring is in place: `apply_order_fulfillment_hold` RPC (migration `20260428000003_order_fulfillment_hold_rpcs.sql`) accepts a `p_commit_lines` parameter and writes to `inventory_commitments` inside the same PL/pgSQL transaction as the `warehouse_orders.fulfillment_hold` stamp. The TS wrapper `src/lib/server/order-hold-rpcs.ts::applyOrderFulfillmentHold({ commitLines })` forwards the array unchanged (see `CommitLine` type). What is still missing is the INGRESS WIRING (same gap as SKU-AUTO-3): `process-client-store-webhook` / `client-store-order-detect` do not yet call `evaluateOrderForHold()` + derive the mixed-order partition, so the `commitLines` path is never exercised against a real order. Integration test with a mixed Shopify order (2 warehouse SKUs + 1 non-warehouse SKU) needs both SKU-AUTO-3's ingress wiring AND the split at the adapter. Flip with SKU-AUTO-3. |
