...
73. **Mega-plan closeout uses an automated spot-check task, not manual verification.** The hourly `megaplan-spot-check` Trigger task (`src/trigger/tasks/megaplan-spot-check.ts`, pinned to the `shipstation` queue) samples 5 SKUs/client daily (15 during ramp window — auto-detected when any workspace has `fanout_rollout_percent < 100`) via the `megaplan_sample_skus_per_client` PL/pgSQL RPC, compares DB / Redis / ShipStation v2 / Bandcamp, and persists artifacts to `megaplan_spot_check_runs`. **Persistence rule (review pass v4 §5.3): drift_major creates a `warehouse_review_queue` row ONLY when the same SKU is drift_major in two consecutive runs** — single-run drift_major is logged on the artifact but suppressed from the queue to eliminate transient propagation noise. Operator surface is `/admin/settings/megaplan-verification`. Do NOT call ShipStation/Bandcamp/Redis from the Server Actions (`src/actions/megaplan-spot-check.ts`) — they only enqueue the task and read persisted results, per Rule #48.

74. **Deferred follow-ups live in `docs/DEFERRED_FOLLOWUPS.md`, not Trello/Linear/issues.** Adding a follow-up = appending a YAML entry with `slug`, `title`, `due_date`, `severity`, `context`. The daily `deferred-followups-reminder` Trigger task (09:00 UTC) parses the file and upserts a `warehouse_review_queue` row per workspace for every entry whose `due_date <= today`. Idempotency is guaranteed by `group_key='deferred-followup-{slug}'` (UNIQUE per migration `20260401000003_scraper_hardening.sql`). The markdown file is bundled into the Trigger.dev runtime via the `additionalFiles` extension declared in `trigger.config.ts` — do NOT rely on `process.cwd()` to find docs in Trigger tasks; always use the `additionalFiles` extension.

## Manual Inventory Count Entry (Saturday Workstream 2)
75. **Manual inventory counts use absolute-set with a confirmation gate, NOT delta entry.** Staff use `/admin/inventory/manual-count` to enter the on-hand quantity directly ("47", not "+3" or "-2"). `submitManualInventoryCounts` (`src/actions/manual-inventory-count.ts`) computes `delta = newAvailable - currentAvailable` server-side. **Confirmation gate**: rows with `|delta| > 10` OR `currentAvailable === 0 && newAvailable > 0` (rising_from_zero) OR `currentAvailable > 0 && newAvailable === 0` (falling_to_zero) return `requires_confirm` and the UI re-submits with `force:true` after operator approval. **Negative-block contract**: `newAvailable < 0` is hard-blocked (the row is NOT written) AND a `warehouse_review_queue` row is upserted (`category='manual_count_negative_block'`, `severity='high'`, `group_key='manual-count.negative-block:{workspaceId}:{sku}'`). Never silently accept a count that produces negative available — it always means an upstream sync bug worth investigating. Per-row writes go through `recordInventoryChange({ source:'manual_inventory_count' })` (Rule #20) with stable `correlationId='manual-count:{userId}:{batchId}:{sku}'` (Rule #15). ShipStation v2 push uses the new sibling task `shipstation-v2-adjust-on-sku` (NOT `shipstation-v2-decrement` — that one is sale-only / decrement-only). Bandcamp + Clandestine Shopify + client-store fanout flows through `fanoutInventoryChange()` automatically. `MAX_ENTRIES_PER_BATCH=200` per Rule #41. Companion test at `tests/unit/actions/manual-inventory-count.test.ts` (Rule #6).

## Inventory Count Sessions + Warehouse-Locations Source-of-Truth (Saturday Workstream 3)
76. **Per-SKU count sessions suppress fanout; only completion fires a single `cycle_count` delta.** A "count session" is the safe primitive for live, on-floor counting. `startCountSession(sku)` (`src/actions/inventory-counts.ts`) flips `warehouse_inventory_levels.count_status` to `count_in_progress` and snapshots `count_baseline_available`. While `count_in_progress`, `setVariantLocationQuantity` writes ONLY to `warehouse_variant_locations` and **does NOT call `recordInventoryChange()`** — Bandcamp / Shopify / ShipStation never see partial counts. `completeCountSession(sku)` re-reads current `available` (Scenario A defense: a sale that lands mid-count is already in `available`, so summing locations and diffing against the *current* available correctly yields delta=0; never diff against `count_baseline_available`), sums per-location quantities, and routes the single resulting delta through `recordInventoryChange({ source:'cycle_count', correlationId:'count-session:{ws}:{sku}:{startedAt}' })` — Rule #20 single write path is preserved. The first per-location write per SKU sets the sticky `has_per_location_data=true` flag — that is the key the deferred per-location rewrite of `shipstation-v2-sync-on-sku` will pivot on. NEVER write to `warehouse_inventory_levels.available` while a session is in progress; NEVER skip the re-read in `completeCountSession`.

77. **`warehouse_locations` is the source of truth; ShipStation v2 inventory locations are a one-way mirror.** Local row creation never blocks on ShipStation (`src/actions/locations.ts` → `createLocation` inserts the local row first, then mirrors via `createInventoryLocation`; 409 conflicts are resolved by `listInventoryLocations` rather than treated as failures). **Renames call ShipStation FIRST** (v4 hardening) — if v2 fails, the local row stays unchanged so the operator can retry against truth. `createLocationRange` enforces Rule #41 by routing >30-entry ranges to the `bulk-create-locations` Trigger task (pinned to the `shipstation` queue, respects `workspaces.shipstation_sync_paused`, surfaces partial mirror failures as one `warehouse_review_queue` row at exit). Never invent a parallel "use ShipStation locations as truth" path — that violates the source-of-truth direction.

78. **Bandcamp unmatched product creation is DB-first; Shopify create is last.** In `src/trigger/tasks/bandcamp-sync.ts`, unmatched merch must follow this order: (1) create `warehouse_products` with `shopify_product_id = NULL`, (2) insert `warehouse_product_variants` and handle `error` explicitly, (3) insert `bandcamp_product_mappings`, (4) seed inventory via `recordInventoryChange`, (5) call Shopify `productSetCreate`, then update `warehouse_products.shopify_product_id`. Never call Shopify before variant insert succeeds — that pattern leaks orphan Shopify drafts when `UNIQUE(workspace_id, sku)` collides. Package-level dedup is `(workspace_id, bandcamp_item_id)` and is enforced by migration `20260419000001_bandcamp_package_dedup_backstop.sql`.

79. **Bandcamp apparel multi-variant create path: per-option variants under one product, single mapping row, never `Title` as the Shopify option name.** When a Bandcamp package exposes ≥2 distinct option SKUs (apparel sizes, color variants, etc.), `bandcamp-sync` MUST detect this via `detectMultiVariantOptions()` (`src/trigger/lib/bandcamp-apparel.ts`) and materialize each option as its own first-class `warehouse_product_variants` row + Shopify variant under one `warehouse_products` row. The Shopify `productOptions` name MUST come from `inferOptionName()` (resolves to `Size` / `Color` / `Variant` — never `Title`, which would silently collapse the variants). Per-option Bandcamp attribution lives on each variant via `warehouse_product_variants.bandcamp_option_id` + `bandcamp_option_title` (migration `20260420000005_variant_bandcamp_option.sql`) — NOT on the mapping row, because `uq_bandcamp_mappings_workspace_item_id` only allows ONE `bandcamp_product_mappings` row per package. Inventory is seeded per option via `recordInventoryChange()` with stable `bandcamp-seed:{band_id}:{package_id}:opt:{option_id}` correlation IDs. After `productSetCreate`, `fetchProductVariantsByProductId()` is called to back-fill `shopify_variant_id` + `shopify_inventory_item_id` per warehouse variant by SKU match. SKU collisions on the new path raise a `bandcamp_apparel_sku_collision` review queue item — NEVER auto-merge into an existing variant. Pre-existing umbrella apparel products (the 616 SKUs the 2026-04-20 audit classifies as `tracked_as_metadata` — recorded inside `bandcamp_product_mappings.bandcamp_option_skus` but not yet first-class variants) are LEFT IN PLACE; restructuring them is a future Path B/C migration that will require coordinated Shopify option restructure + inventory re-allocation (umbrella variants may be referenced by past customer orders).
